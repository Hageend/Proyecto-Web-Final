...
class MapAdapter:
    ...
    def adjust_environment_params(self, environment_type, winning_map_metrics):
        ...
        # Obtenemos el nivel de caos del mapa ganador
        winning_chaos = winning_map_metrics.get('chaos', 0)
        
        # CASO 1: Al usuario le gusta el CAOS (> 0.15)
        # Acción: Reducir el suavizado (Cellular Automata) para dejar bordes rugosos
        if winning_chaos > 0.15:
            # Reducimos los pases de suavizado (mínimo 1)
            current_passes = env_adjustments.get('cellular_automata_passes', 3) # 3 es el default en la.py
            env_adjustments['cellular_automata_passes'] = max(1, current_passes - 1)
            
            # Para Forest: Aumentamos el peso del "random_walk" (caminata aleatoria)
            if environment_type == 'forest':
                 env_adjustments['random_walk_weight'] = min(10, env_adjustments.get('random_walk_weight', 4) + 1)

        # CASO 2: Al usuario le gusta el ORDEN (< 0.08)
        # Acción: Aumentar el suavizado para limpiar el mapa
        elif winning_chaos < 0.08:
            # Aumentamos pases de suavizado (máximo 5)
            current_passes = env_adjustments.get('cellular_automata_passes', 3)
            env_adjustments['cellular_automata_passes'] = min(5, current_passes + 1)
            
            # Para Forest: Reducimos el random walk
            if environment_type == 'forest':
                 env_adjustments['random_walk_weight'] = max(1, env_adjustments.get('random_walk_weight', 4) - 1)
